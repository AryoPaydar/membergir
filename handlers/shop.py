...
async def shop_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=inline([
            [("💰 خرید الماس", "shop_coin")],
            [("♻️ خرید پنل", "shop_panel")],
        ])
    )


# ==================== منوی فروشگاه از callback ====================
async def shop_menu_from_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """نمایش منوی فروشگاه از داخل callback (وقتی روی دکمه خرید الماس کلیک میشه)"""
    q = update.callback_query
    user_id = q.from_user.id
    user = get_user(user_id)
    
    if not user:
        await q.message.reply_text("❌ لطفاً ابتدا /start را بزنید.")
        return
    
    # چک تأیید شماره
    if not user.get("phone"):
        set_user_state(user_id, "shop_phone")
...
async def handle_state(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    await msg.reply_text(
        "✅ شماره تأیید شد. حالا از فروشگاه خرید کنید.",
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if data == "shop_coin":
        ...
        with db.conn() as c:
            items = c.execute(
                "SELECT * FROM shop_items WHERE item_type='coin' ORDER BY position"
            ).fetchall()
        
        if not items:
            await q.message.reply_text("❌ هنوز آیتم سکه‌ای تنظیم نشده.")
            return True
        
        rows = []
        for it in items:
            rows.append([
                (f"{it['name']} | {it['coin_amount']} سکه | {it['price']:,} ریال",
                 f"buy_coin:{it['id']}")
            ])
        rows.append([("🔙 بازگشت", "shop_back")])
    ...
    if data == "shop_panel":
        ...
        with db.conn() as c:
            items = c.execute(
                "SELECT * FROM shop_items WHERE item_type='panel' ORDER BY position"
            ).fetchall()
        
        if not items:
            await q.message.reply_text("❌ هنوز آیتم پنلی تنظیم نشده.")
            return True
        
        rows = []
        for it in items:
            rows.append([
                (f"{it['name']} | {it['panel_name']} {it['panel_days']} روز | {it['price']:,} ریال",
                 f"buy_panel:{it['id']}")
            ])
        rows.append([("🔙 بازگشت", "shop_back")])
