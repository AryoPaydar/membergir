...
# ==================== سفارشات کاربر از ReplyKeyboard ====================
async def user_orders_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db.conn() as c:
        ...
        if not orders:
            await update.message.reply_text(
                ...
                reply_markup=inline([
                    [("🔙 بازگشت به منوی اصلی", "tracking_back")]
                ])
            )
            return
        ...
        for o in orders:
            ...
            left = c.execute(
                ...
                SELECT COUNT(*) c
                FROM order_members
                WHERE order_id = ? AND left_at IS NOT NULL
                """,
                (order_id,)
            ).fetchone()["c"]
        ...
        orders = c.execute("""
            SELECT o.*, 
                   (SELECT COUNT(*) 
                    ...
                    AND om.left_at IS NULL) as active_members
            FROM orders o
            WHERE o.admin_id = ?
            ORDER BY o.id DESC
            LIMIT 20
...
async def order_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if order["status"] == "running":
        if get_setting("cancel_enabled", "on") == "on":
            rows.append([
                ("❌ لغو سفارش", f"cancel_confirm:{order_id}")
            ])
...
async def cancel_order_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    min_members = int(
        get_setting("cancel_min_members", "100")
    )
...
async def cancel_order_confirm(
    ...
    # محاسبه بازگشت
    ratio = float(
        get_setting("cancel_refund_ratio", "0.5")
    )

    remaining = (
        order["member_target"] -
        order["member_received"]
    )
...
async def cancel_order_do(
    ...
    with db.conn() as c:
        ...
        c.execute(
            ...
            (to_id, amount, type, description)
            VALUES (?, ?, 'order_cancel_refund', ?)
            """,
            (
                user_id,
                refund,
                f"بازگشت از سفارش #{order_id}"
            )
        )
    ...
    await q.answer(
        f"✅ سفارش لغو شد. {refund:,} سکه بازگشت.",
        show_alert=True
    )
